...
/* =======================
   ...
   - KEEP the original 'start' element style: use anchor with class 'task-link' (as in HTML)
======================= */
...
function renderTasks(tasks) {
  ...
  tasks.forEach(task => {
    ...
    if (task.claimed) {
      // don't render tasks already claimed
      return;
    }
    ...
    // Create anchor (keep original element type/class from HTML)
    const startLink = document.createElement('a');
    startLink.className = "task-link";
    startLink.href = task.link || '#';
    startLink.target = "_blank";
    startLink.textContent = "start";
    startLink.dataset.taskId = task.id;
    startLink.style.cursor = "pointer";

    // Click handler: open link in new tab, then attempt claim; do not replace the element,
    // only change textContent and background color to show checked state.
    startLink.addEventListener('click', async function (e) {
      e.preventDefault();

      // Prevent multiple clicks
      if (startLink.dataset.processing === '1') return;
      startLink.dataset.processing = '1';

      // Open the link in a new tab
      if (startLink.href && startLink.href !== '#') {
        try {
          window.open(startLink.href, '_blank');
        } catch (err) {}
      }

      const originalText = startLink.textContent;
      // Show immediate feedback
      startLink.textContent = "checking...";
      startLink.style.pointerEvents = 'none';

      try {
        const claimRes = await fetchApi({
          type: "claimTask",
          data: { taskId: startLink.dataset.taskId }
        });
        ...
        if (claimRes && claimRes.success) {
          ...
          // Change text content only and apply checked background color
          startLink.textContent = "✔";
          startLink.style.background = "green";
          startLink.style.color = "#fff";
          ...
          // Remove the task card for this user after a short delay
          setTimeout(() => {
            try { card.remove(); } catch (e) {}
          }, 800);

        } else {
          // Failure: restore state and show error
        ...
        startLink.dataset.processing = '0';
      }
    });
