...
async function handleBoxClick(evt) {
 ...
 // Both ads shown: award random reward immediately (no API delay)
 const reward = BOX_REWARDS[Math.floor(Math.random() * BOX_REWARDS.length)];

 // Show notification immediately before API call for instant feedback
 try {
 ...
 // Try server-side reward in background (non-blocking)
 try {
   const res = await fetchApi({ type: "rewardBox", data: { amount: reward } });
   if (res && res.success) {
     // server returned updated balance - sync it
     ADS = Number(res.balance) || ADS;
     if (adsBalance) adsBalance.textContent = ADS;
     if (walletbalance) {
       walletbalance.innerHTML = `
     ...
     // If server didn't support rewardBox, fallback to rewardUser with isBox flag
     console.warn("rewardBox failed or not supported, attempting fallback. Server response:", res);
     try {
       ...
       console.warn("fallback rewardUser failed:", e);
     }
   }
 } catch (e) {
